    ...
    while( !FireTimerIsSimCurYearTimeExpired(ft) )
    {
      ...
      */
      for ( exp_secs = 0.0; exp_secs < (double) timestep; exp_secs += iter_secs ) 
      {
        ...
        for( lel = LIST_HEAD(brn_cells_list), prev = NULL; lel != NULL; prev = lel, lel = LIST_GET_NEXT_ELMT(lel) )
        {
...
#if 0
        /* compute neighbor cell contribution */
        for( lel = LIST_HEAD(brn_cells_list); lel != NULL; lel = LIST_GET_NEXT_ELMT(lel) )
        {
          ...
          i = brn_cell->i;
          j = brn_cell->j;
          /* skip neighbor contribution for circular fires */
          if ( brn_cell->eccen < DBL_EPSILON )
          {
            continue;
          }
          /* compute the neighbor and a burning direction for cells along the heading portion of fire */
          switch ( brn_cell->max_ros_az )
          {
          case 0:
            //nbr_az[0] = 6; brn_az[0] = 1;
            nbr_az[0] = 7; brn_az[0] = 2;
            nbr_az[1] = 0; brn_az[1] = 0;
            nbr_az[2] = 1; brn_az[2] = 6;
            //nbr_az[2] = 2; brn_az[2] = 7;
            break;
          case 1:
            nbr_az[0] = 0; brn_az[0] = 2;
            nbr_az[1] = 1; brn_az[1] = 1;
            nbr_az[2] = 2; brn_az[2] = 0;
            break;
          case 2:
            //nbr_az[0] = 0; brn_az[0] = 3;
            nbr_az[0] = 1; brn_az[0] = 4;
            nbr_az[1] = 2; brn_az[1] = 2;
            nbr_az[2] = 3; brn_az[2] = 0;
            //nbr_az[2] = 4; brn_az[2] = 1;
            break;
          case 3:
            nbr_az[0] = 2; brn_az[0] = 4;
            nbr_az[1] = 3; brn_az[1] = 3;
            nbr_az[2] = 4; brn_az[2] = 2;
            break;
          case 4:
            //nbr_az[0] = 2; brn_az[0] = 5;
            nbr_az[0] = 3; brn_az[0] = 6;
            nbr_az[1] = 4; brn_az[1] = 4;
            nbr_az[2] = 5; brn_az[2] = 2;
            //nbr_az[2] = 6; brn_az[2] = 3;
            break;
          case 5:
            nbr_az[0] = 4; brn_az[0] = 6;
            nbr_az[1] = 5; brn_az[1] = 5;
            nbr_az[2] = 6; brn_az[2] = 4;
            break;
          case 6:
            //nbr_az[0] = 4; brn_az[0] = 7;
            nbr_az[0] = 5; brn_az[0] = 0;
            nbr_az[1] = 6; brn_az[1] = 6;
            nbr_az[2] = 7; brn_az[2] = 4;
            //nbr_az[2] = 0; brn_az[2] = 5;
            break;
          case 7:
            nbr_az[0] = 6; brn_az[0] = 0;
            nbr_az[1] = 7; brn_az[1] = 7;
            nbr_az[2] = 0; brn_az[2] = 6;
            break;
          }
          /* compute the contribution from the neighboring cells along the heading portion of the fire */
          for (cell = 0; cell < 3; cell++)
          {
            /* retrieve coordinate information for neighbor cell */
            nbr_i = EIGHTNBR_ROW_INDEX_AT_AZIMUTH(i, nbr_az[cell]);
            nbr_j = EIGHTNBR_COL_INDEX_AT_AZIMUTH(j, nbr_az[cell]);
            /* skip computation for boundary cells */
            if ( nbr_i == 0 || nbr_j == 0 || nbr_i == (domain_rows - 1) || nbr_j == (domain_cols - 1) )
            {
              continue;
            ...
            case EnumHasFireCellState:
              /* cell is already burning, compute the contribution from the burning cell */
              nbr_brn_cell = brn_cells_map[nbr_i * domain_cols + nbr_j];
              if ( nbr_brn_cell != NULL  && nbr_brn_cell->max_rosmps > 0.0 )
              {
                brn_cell->distm[brn_cell->max_ros_az] += nbr_brn_cell->rosmps[brn_az[cell]] * iter_secs;
              }
              break;
            case EnumNoFireCellState:
              /* cell is not burning, do nothing */
              break;
            }
          }
        }
