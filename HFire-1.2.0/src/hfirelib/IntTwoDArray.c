...
IntTwoDArray * InitIntTwoDArraySizeEmpty(unsigned int num_rows, unsigned int num_cols)	{
	...
	if ( (ia = (IntTwoDArray *) malloc(sizeof(IntTwoDArray))) == NULL )	{
		ERR_ERROR_CONTINUE("Unable to initialize array, memory allocation failed. \n", ERR_ENOMEM);
		return ia;
		}
	if ( (ia->array = (int **) malloc(sizeof(int *) * num_rows)) == NULL ) {
		ERR_ERROR_CONTINUE("Unable to initialize array, memory allocation failed. \n", ERR_ENOMEM);
		free(ia);
		ia = NULL;
		return ia;
	...
	for(i = 0; i < num_rows; i++)	{
		if ( (ia->array[i] = (int *) malloc(sizeof(int) * num_cols)) == NULL )	{
			ERR_ERROR_CONTINUE("Unable to initialize array, memory allocation failed. \n", ERR_ENOMEM);
...
IntTwoDArray * InitIntTwoDArraySizeIniValue(unsigned int num_rows, unsigned int num_cols, int initial_value)	{
	int i, j;
	IntTwoDArray * ia = InitIntTwoDArraySizeEmpty(num_rows, num_cols);
	if ( ia == NULL )	{
...
int GetSizeColIntTwoDArray(IntTwoDArray * arr)	{
	...
	return INTTWODARRAY_SIZE_COL(arr);
	}
...
int GetSizeRowIntTwoDArray(IntTwoDArray * arr)	{
	...
	return INTTWODARRAY_SIZE_ROW(arr);
	}
...
int GetDataIntTwoDArray(IntTwoDArray * arr, unsigned int row, unsigned int col, int * data)	{
	...
	*data = INTTWODARRAY_GET_DATA(arr, row, col);
	return ERR_SUCCESS;
	}
...
int SetDataIntTwoDArray(IntTwoDArray * arr, unsigned int row, unsigned int col, int data)	{
	...
	INTTWODARRAY_SET_DATA(arr, row, col, data);
	return ERR_SUCCESS;
	}
